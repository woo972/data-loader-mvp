...
public interface WeatherRepository extends JpaRepository<Weather, Long> {
    @Query(value = "SELECT weather_id, weather_data FROM weather" +
            " WHERE json_value(weather_data,'$.date') >= :start_date" +
            " AND json_value(weather_data,'$.date') <= :end_date" +
            " AND json_value(weather_data, '$.location_code') = :location_code" +
            " ORDER BY json_value(weather_data, '$.date')",
